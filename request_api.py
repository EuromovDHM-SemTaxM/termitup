...
from modules_api.enrichment import term_id, contextCode, iateCode, eurovocCode, unescoCode, wikidataCode, thesozCode, \
    stwCode, iloCode, activateRelval
from modules_api.postprocessing.modules.accents import AccentRemovalPostProcessor
from modules_api.postprocessing.modules.numbers import NumberRemovalPostProcessor
from modules_api.postprocessing.modules.pattern import PatternCleaningProcessor
from modules_api.postprocessing.modules.plurals import PluralPostProcessor
from modules_api.postprocessing.modules.timex import TimeExPostProcessor
from modules_api.postprocessing.termpostprocessor import TermPostProcessorRegistry
from modules_api.termextraction import Term

from modules_api.termextraction.modules.tbx import TBXTermExtractor
from modules_api.termextraction.modules.termsuite import TermsuiteTermExtractor
from modules_api.termextraction.termextractor import TermExtractorRegistry
from modules_api.validation import relvalCode
from modules_api.publication import rmlCode

REQUEST_API = Blueprint("term_api", __name__)

logger = logging.getLogger("TermItUP REST API")
...
term_extractor_registry = TermExtractorRegistry()
term_extractor_registry.register(
    [
        TermsuiteTermExtractor('method eq "termsuite"'),
        TBXTermExtractor('method eq "tbxtools"')
    ])
...
def extract_terminology():
    ...
    logger.info(f"Received: {len(corpus)} character corpus in {language}")

    terminology = term_extractor_registry(corpus, json_data)

    return Response(json.dumps(terminology), mimetype="application/json")


# On a production WSGI server such as GUNICORN, enable preload, disable reload
postprocessor_registry = TermPostProcessorRegistry()
postprocessor_registry.register(
    [
        PatternCleaningProcessor('source_language eq "es" and tasks contains "patterns"'),
        TimeExPostProcessor('tasks contains "timeEx"'),
        PluralPostProcessor('source_language eq "es" and tasks contains "plurals"'),
        NumberRemovalPostProcessor('tasks contains "numbers"'),
        AccentRemovalPostProcessor('tasks contains "accents"')
    ])
...
def postproc_terminology():
    ...
    logger.info(f"Received: {str(terms)} {str(tasks)} {language}")

    clean_terms = postprocessor_registry(terms, json_data)
...
def enrinching_terminology():
    ...
    for t in termlist:
        ...
        if sparql == 'yes':
            ...
            subprocess.call(
                "/opt/virtuoso/termitup/bin/isql -S 1111 -U termitup -P " + password + " verbose=on banner=off prompt=off echo=ON errors=stdout exec=\"DB.DBA.TTLP_MT(file_to_string_output ('/opt/data/tmp.ntriples'), '', '" +
                json_data["schema_name"] + "',0); checkpoint;\"", shell=True)
...
def enrinching_terminology_internal(json_data):
    ...
    if sparql == 'yes':
        ...
        passw = open('password.txt', 'r')
        password = passw.read()
        call = "/opt/virtuoso/termitup/bin/isql -S 1111 -U termitup -P " + password + " verbose=on banner=off prompt=off echo=ON errors=stdout exec=\"DB.DBA.TTLP_MT(file_to_string_output ('/opt/data/tmp.ntriples'), '', '" + \
               json_data["schema_name"] + "',0); checkpoint;\""
        print('this is call')
        print(call)
