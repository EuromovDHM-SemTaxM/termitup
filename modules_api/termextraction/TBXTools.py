...
class TBXTools:
    ...
    def create_project(self, project_name, sl_lang, tl_lang="null", overwrite=False):
        ...
        if project_name != ":momory:" and os.path.isfile(project_name) and not overwrite:
            raise Exception("This file already exists")
        else:
            if project_name != ":momory:" and os.path.isfile(project_name) and overwrite:
            ...
            with self.conn:
                ...
                self.cur.execute("INSERT INTO configuration (id, sl_lang, tl_lang) VALUES (?,?,?)",
        ...
        if format == "TBXTools":
            ...
            for self.line in self.cf:
                ...
                if self.line == "":
                    ...
                    self.data.append(self.record)
                    if self.continserts == self.maxinserts:
                        self.cur.executemany("INSERT INTO sl_tagged_corpus (id, tagged_segment) VALUES (?,?)",
                                             self.data)
                        self.data = []
                        self.continserts = 0
                        self.data = []
    ...
    def show_term_candidates(self, limit=-1, minfreq=2, minmeasure=-1, show_frequency=True, show_measure=False,
                             mark_eval=False, verbose=False):
        ...
        with self.conn:
            self.cur.execute(
                "SELECT frequency,measure,n,candidate FROM term_candidates order by frequency desc limit " + str(limit))
    ...
    def generate_term_candidates(self, limit=-1, minfreq=2, minmeasure=-1, show_frequency=True,
                                 show_measure=False,
                                 mark_eval=False, verbose=False):
        ...
        with self.conn:
            self.cur.execute(
                "SELECT frequency,value,n,candidate FROM term_candidates order by value desc, frequency desc, random() limit " + str(
                    limit))
            # self.cur.execute("SELECT frequency,value,n,candidate FROM term_candidates order by n desc limit "+str(limit))
            for candidate_term in self.cur.fetchall():
                self.frequency = candidate_term[0]
                ...
                self.n = candidate_term[2]
                self.candidate = candidate_term[3]
                if self.n_min <= self.n <= self.n_max and not self.candidate in self.knownterms:
                    if mark_eval:
                        if self.candidate in self.evaluation_terms:
                            self.candidate = "*" + self.candidate
    ...
    def save_term_candidates(self, outfile, limit=-1, minfreq=2, minmeasure=-1, show_frequency=True, show_measure=False,
                             mark_eval=False, verbose=False):
        '''Saves the term candidates in a file.'''
        out = self.generate_term_candidates(limit, minfreq, minmeasure, show_frequency, show_measure, mark_eval,
                                            verbose)
    ...
    def ngram_calculation(self, nmin, nmax, minfreq=2):
        ...
        with self.conn:
            ...
            for self.s in self.cur.fetchall():
                ...
                for self.n in range(nmin,
                                    nmax + 2):  # we calculate one order bigger in order to detect nested candidates
    ...
    def statistical_term_extraction(self, minfreq=2):
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO term_candidates (candidate, n, frequency, measure, value) VALUES (?,?,?,?,?)", self.data)
            self.conn.commit()
    ...
    def nest_normalization(self, percent=10, verbose=False):
        ...
        for self.a in self.results:
            ...
            self.cur.execute("SELECT candidate,frequency FROM term_candidates where frequency <=" + str(
                self.fmax) + " and frequency>=" + str(self.fmin) + "  and n =" + str(self.nb) + " and n<=" + str(
                self.n_max))
    ...
    def evaluate_pos(self, limit, order="desc", iterations=1000, ignore_case=True):
        ...
        with self.conn:
            for i in range(0, iterations):
                if order == "desc":
                    self.cur.execute("SELECT candidate from term_candidates where n<=" + str(
                        self.n_max) + " order by value desc, frequency desc, random() limit " + str(limit))
                elif order == "asc":
                    self.cur.execute("SELECT candidate from term_candidates where n<=" + str(
                        self.n_max) + " order by value asc, frequency desc, random() limit " + str(limit))
                else:
    ...
    def index_phrase_table(self, phrasetable):
        ...
        while 1:
            ...
            if not self.trad[0] in self.punctuation and not self.source[0] in self.punctuation and not self.trad[
                                                                                                           -1] in self.punctuation and not \
                    self.source[-1] in self.punctuation:
    ...
    def find_translation_ptable(self, sourceterm, maxdec=1, maxinc=1, ncandidates=5, separator=":"):
        ...
        for self.a in self.results:
            ...
            if not self.tttokens[0] in self.tl_stopwords and not self.tttokens[-1] in self.tl_stopwords and len(
                    self.tttokens) >= len(sourceterm.split(" ")) - maxdec and len(self.tttokens) <= len(
                sourceterm.split(" ")) + maxinc:
                ...
                if self.continserts == self.maxinserts:
                    if corpus == "source":
                        self.cur.executemany("INSERT INTO sl_tagged_corpus (id, tagged_segment) VALUES (?,?)",
                                             self.data)
                    if corpus == "target":
                        self.cur.executemany("INSERT INTO tl_tagged_corpus (id, tagged_segment) VALUES (?,?)",
                                             self.data)
                    self.data = []
                    self.continserts = 0
            with self.conn:
    ...
    def tagged_ngram_calculation(self, nmin=2, nmax=3, minfreq=2):
        ...
        for self.c in self.ngrams.most_common():
            if self.c[1] >= minfreq:
                ...
                self.data.append(self.record)
        with self.conn:
            self.cur.executemany("INSERT INTO tagged_ngrams (ngram, tagged_ngram, n, frequency) VALUES (?,?,?,?)",
                                 self.data)
            self.conn.commit()
    ...
    def learn_linguistic_patterns(self, outputfile, showfrequencies=False, encoding="utf-8", verbose=True,
    ...
    def tsr(self, termsfile, encoding="utf-8", type="strict",
            max_iterations=10000000000):  # IMPLEMENTED ONLY FOR BIGRAMS
        self.component = {}
        self.firstcomponent = {}
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO term_candidates (candidate, n, frequency, measure, value) VALUES (?,?,?,?,?)", data)
...
class myBigramAssocMeasures(nltk.collocations.BigramAssocMeasures):
    ...
    def chi_sq_2g(self, n_ii, n_ix_xi_tuple, n_xx):
        ...
        x2 = 2 * (((n11 - m11) / m11) ** 2 + ((n12 - m12) / m12) ** 2 + ((n21 - m21) / m21) ** 2 + (
                (n22 - m22) / m22) ** 2)
        return (x2)
    ...
    def loglikelihood_2g(self, n_ii, n_ix_xi_tuple, n_xx):
        ...
        try:
            LogLikelihood = 2 * (
                    n11 * math.log((n11 / m11), 2) + n12 * math.log((n12 / m12), 2) + n21 * math.log((n21 / m21),
                                                                                                     2) + n22 * math.log(
                (n22 / m22), 2))
        except:
            LogLikelihood = 0
    ...
    def tmi_2g(self, n_ii, n_ix_xi_tuple, n_xx):
        ...
        m22 = (n2p * np2 / npp)
        try:
            tmi = ((n11 / npp) * math.log((n11 / m11), ) + (n12 / npp) * math.log((n12 / m12), 2) + (
                    n21 / npp) * math.log((n21 / m21), 2) + (n22 / npp) * math.log((n22 / m22), 2))
        except:
            tmi = 0
        return (tmi)
    ...
    def log_likelihood_ratio_TEXTNSP(self, n_ii, n_ix_xi_tuple, n_xx):
        ...
        try:
            self.LLR = 2 * ((n_ii * math.log((n_ii / m11), 2)) + (n_oi * math.log((n_oi / m12), 2)) + (
                    n_io * math.log((n_io / m21), 2)) + (n_oo * math.log((n_oo / m22), 2)))
        except:
            self.LRR = 0
    ...
    def tmi(self, n_ii, n_ix_xi_tuple, n_xx):
        ...
        self.tmi = (n_ii / n_xx) * self.PMI + (n_oi / n_xx) * self.PMI + (n_io / n_xx) * self.PMI + (
                n_oo / n_xx) * self.PMI
...
class myTrigramAssocMeasures(nltk.collocations.TrigramAssocMeasures):
    ...
    def tmi_3g(cls, *dades):
        ...
        try:
            tmi = (n111 / nppp * math.log((n111 / m111), 2) + n112 / nppp * math.log((n112 / m112),
                                                                                     2) + n121 / nppp * math.log(
                (n121 / m121), 2) + n122 / nppp * math.log((n122 / m122), 2) + n211 / nppp * math.log((n211 / m211),
                                                                                                      2) + n212 / nppp * math.log(
                (n212 / m212), 2) + n221 / nppp * math.log((n221 / m221), 2) + n222 / nppp * math.log((n222 / m222), 2))
        except:
            tmi = 0
    ...
    def loglikelihood_3g(cls, *dades):
        ...
        try:
            Log_Likelihood = 2 * (
                    n111 * math.log((n111 / m111), 2) + n112 * math.log((n112 / m112), 2) + n121 * math.log(
                (n121 / m121), 2) + n122 * math.log((n122 / m122), 2) + n211 * math.log((n211 / m211),
                                                                                        2) + n212 * math.log(
                (n212 / m212), 2) + n221 * math.log((n221 / m221), 2) + n222 * math.log((n222 / m222), 2))
        except:
            Log_Likelihood = 0
